...
class HDF5Format(Formatter):
    ...
    def close_file(self, data_set: "qcodes_loop.data.data_set.DataSet"):
        """
    ...
    def _open_file(self, data_set, location=None):
        ...
        filepath = self._filepath_from_location(location, io_manager=data_set.io)
        data_set._h5_base_group = h5py.File(filepath, "r+")

    def read(self, data_set: "qcodes_loop.data.data_set.DataSet", location=None):
        """
        ...
        for i, array_id in enumerate(data_set._h5_base_group["Data Arrays"].keys()):
    ...
    def _filepath_from_location(self, location, io_manager):
        ...
        filepath = io_manager.to_path(location + f"/{filename}.hdf5")
        return filepath

    def _create_data_object(self, data_set, io_manager=None, location=None):
    ...
    def write(
        ...
        for array_id in data_set.arrays.keys():
            if array_id not in arr_group.keys() or force_write:
                self._create_dataarray_dset(
                    array=data_set.arrays[array_id], group=arr_group
                )
            ...
            new_datasetshape = (new_dlen, datasetshape[1])
    ...
    def _create_dataarray_dset(self, array, group):
        ...
        for i in range(len(array.set_arrays)):
            set_arrays += [_encode_to_utf8(str(array.set_arrays[i].array_id))]
        dset.attrs["set_arrays"] = set_arrays

        return dset

    def write_metadata(
        self, data_set, io_manager=None, location=None, read_first=True, **kwargs
    ):
        """
    ...
    def _read_list_group(self, entry_point, list_type):
        d = {}
        self.read_dict_from_hdf5(data_dict=d, h5_group=entry_point[list_type])
    ...
    def _write_list_group(self, key, item, entry_point, list_type):
        ...
        self.write_dict_to_hdf5(data_dict=item, entry_point=entry_point[key][list_type])
                        ...
                        if isinstance(item[0], (int, float, np.int32, np.int64)):
                            ...
                            for i, list_item in enumerate(item):
                                ...
                                self.write_dict_to_hdf5(
                                    data_dict=list_item, entry_point=list_item_grp
                                )
                        else:
                            logging.warning(
        ...
        if "list_type" not in h5_group.attrs:
            for key, item in h5_group.items():
                if isinstance(item, h5py.Group):
                    data_dict[key] = {}
                    data_dict[key] = self.read_dict_from_hdf5(data_dict[key], item)
