...
class MatPlot(BasePlot):
    ...
    def __init__(
        self, *args, figsize=None, interval=1, subplots=None, num=None, **kwargs
    ):
    ...
    def _make_args_for_pcolormesh(args_masked, x, y):
        ...
        if x is not None and y is not None:
            ...
            for k, arr in enumerate(args_masked[:-1]):
                ...
                arr.unshare_mask()
                arr[arr.mask] = np.linspace(
                    extrapolation_start, extrapolation_stop, num=n_invalid
                )
    ...
    def _draw_pcolormesh(
        ...
        **kwargs,
        ...
        args_masked = [masked_invalid(arg) for arg in [x, y, z] if arg is not None]
    ...
    def rescale_axis(self):
        ...
        for i, subplot in enumerate(self.subplots):
            traces = [
                trace
                for trace in self.traces
                if trace["config"].get("subplot", None) == i + 1
            ]
            if not traces:
                continue
            else:
                ...
                trace = traces[0]
            for axis in "x", "y", "z":
                if axis in trace["config"] and isinstance(
                    trace["config"][axis], DataArray
                ):
                    ...
                    if unit in units_to_scale:
                        scale = 1
                        new_unit = unit
                        for prefix, threshold, trialscale in zip(
                            prefixes, thresholds, scales
                        ):
                            if maxval < threshold:
                                scale = trialscale
                        ...
                        tx = ticker.FuncFormatter(partial(scale_formatter, scale=scale))
