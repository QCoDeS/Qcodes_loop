...
class TestSafeFormatter(TestCase):
    def test_normal_formatting(self):
        ...
        self.assertEqual(
            formatter.format("{apples}{{{oranges}}}", apples="red", oranges="unicorn"),
            "red{unicorn}",
        )
...
def _default(time: datetime, formatter: FormatLocation, counter: str, name: str):
    ...
    fmted = formatter.formatter.format(
        formatter.default_fmt, date=date, counter=counter, time=mytime, name=name
    )
    return fmted
...
class TestFormatLocation(TestCase):
    def test_default(self):
        ...
        name = "name"
        self.assertEqual(
            lp(MatchIO([]), {"name": name}), _default(datetime.now(), lp, "001", name)
        )
    ...
    def test_fmt_subparts(self):
        ...
        self.assertEqual(lp(MatchIO([])), datetime.now().strftime(fmt))
        self.assertEqual(
            lp(MatchIO([]), {"name": "who?"}), datetime.now().strftime(fmt) + "_who?"
        )

        self.assertEqual(lp(MatchIO([""])), datetime.now().strftime(fmt) + "_##2.0~")
        self.assertEqual(
            lp(MatchIO(["", "9"])), datetime.now().strftime(fmt) + "_##10.0~"
        )
        self.assertEqual(
            lp(MatchIO(["", "12345"]), {"name": "you!"}),
    ...
    def test_record_call(self):
        ...
        self.assertEqual(lp(MatchIO([]), {"name": "Joe", "label": "fruit"}), expected)

        expected = datetime.now().strftime("%Y-%m-%d/#1000_%H-%M-%S_Ga_As")
        self.assertEqual(lp(MatchIO(["999"]), {"name": "Ga", "label": "As"}), expected)
