...
class RecordingMockFormatter:
    ...
    def write_metadata(self, data_set, io_manager, location, read_first=True):
        self.write_metadata_calls.append(
            (io_manager.base_location, location, read_first)
        )
...
def DataSet1D(location=None, name=None):
    ...
    y = DataArray(
        name="y", label="Y", preset_data=(3.0, 4.0, 5.0, 6.0, 7.0), set_arrays=(x,)
    )
...
def DataSet2D(location=None, name=None):
    ...
    y = DataArray(
        name="y", label="Y", preset_data=yy, set_arrays=(x,), is_setpoint=True
    )
...
def file_1d():
    return "\n".join(
        ["# x_set\ty", '# "X"\t"Y"', "# 5", "1\t3", "2\t4", "3\t5", "4\t6", "5\t7", ""]
    )


def DataSetCombined(location=None, name=None):
    # Complex DataSet with two 1D and two 2D arrays
    x = DataArray(name="x", label="X!", preset_data=(16.0, 17.0), is_setpoint=True)
    y1 = DataArray(
        name="y1", label="Y1 value", preset_data=(18.0, 19.0), set_arrays=(x,)
    )
    y2 = DataArray(
        name="y2", label="Y2 value", preset_data=(20.0, 21.0), set_arrays=(x,)
    )

    yset = DataArray(
        name="y", label="Y", preset_data=(22.0, 23.0, 24.0), is_setpoint=True
    )
    yset.nest(2, 0, x)
    z1 = DataArray(
        name="z1",
        label="Z1",
        preset_data=((25.0, 26.0, 27.0), (28.0, 29.0, 30.0)),
        set_arrays=(x, yset),
    )
    z2 = DataArray(
        name="z2",
        label="Z2",
        preset_data=((31.0, 32.0, 33.0), (34.0, 35.0, 36.0)),
        set_arrays=(x, yset),
    )
    return new_data(arrays=(x, y1, y2, yset, z1, z2), location=location, name=name)
