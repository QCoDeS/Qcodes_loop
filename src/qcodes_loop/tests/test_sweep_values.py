...
def test_snapshot(c0) -> None:
    ...
    assert sw0[0].snapshot() == {"parameter": c0.snapshot(), "values": [{"item": 0}]}

    assert sw0[0:5:0.3].snapshot()["values"] == [
        {"first": 0, "last": 4.8, "num": 17, "type": "linear"}
    ]

    sv = sw0.sweep(start=2, stop=4, num=5)
    assert sv.snapshot()["values"] == [
        {"first": 2, "last": 4, "num": 5, "type": "linear"}
    ]

    # mixture of bare items, nested lists, and slices
    sv = sw0[1, 7, 3.2, [1, 2, 3], 6:9:1, -4.5, 5.3]
    assert sv.snapshot()["values"] == [
        {"first": 1, "last": 5.3, "min": -4.5, "max": 8, "num": 11, "type": "sequence"}
    ]

    assert (sw0[0] + sw0[1]).snapshot()["values"] == [{"item": 0}, {"item": 1}]

    assert (sw0[0:3:1] + sw0[4, 6, 9]).snapshot()["values"] == [
...
def test_repr(c0) -> None:
    sw0 = Sweeper(c0)
    sv = sw0[0]
    assert repr(sv) == (f"<qcodes_loop.sweep_values.SweepFixedValues: c0 at {id(sv)}>")
