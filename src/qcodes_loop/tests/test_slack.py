...
def setup_slack():
    ...
    import qcodes_loop.extensions.slack  # pylint: disable=import-outside-toplevel

    slack = qcodes_loop.extensions.slack.Slack(config=slack_config, auto_start=False)
...
def test_convert_command_should_convert_floats():
    ...
    cmd, arg, kwarg = qcodes_loop.extensions.slack.convert_command("comm 0.234 key=0.1")
    assert cmd == "comm"
...
def test_slack_instance_should_get_config_from_qc_config():
    ...
    slack = qcodes_loop.extensions.slack.Slack(config=None, auto_start=False)
    assert "dummyuser" in slack.users.keys()
...
def test_slack_instance_should_start(mocker):
    ...
    _ = qcodes_loop.extensions.slack.Slack(config=slack_config)
...
def test_slack_instance_should_start_and_stop(mocker):
    ...
    slack = qcodes_loop.extensions.slack.Slack(config=slack_config, interval=0)
    slack.stop()
...
def test_slack_instance_should_get_im_ids(mock_webclient):
    def conversations_history(channel, limit=None):
        if channel == "CH234":
            messages = [
                {"user": "DU123", "text": f"m{i}", "ts": f"{45.5 + i}"}
...
def test_slack_instance_should_get_im_messages_w_count(slack, mock_webclient):
    def conversations_history(channel, limit=None):
        if channel == "CH234":
            messages = [{"user": "DU123", "text": f"message{i}"} for i in range(limit)]
            response = {"messages": messages}
            return response
        else:
            return None
...
def test_slack_instance_should_update_with_task_returning_true(slack, mocker):
    mocker.patch("qcodes_loop.extensions.slack.active_loop", return_value=not None)
...
def test_slack_instance_should_update_with_exception(slack, mocker):
    method_name = "qcodes_loop.extensions.slack.Slack.get_new_im_messages"
...
def test_slack_instance_should_give_help_message(slack):
    ...
    expected_message = (
        "\nAvailable commands: `plot`, `msmt`, `measurement`, `notify`, `help`, `task`"
    )
...
def test_slack_instance_should_handle_messages(mock_webclient, slack):
    ...
    expected_output = {"channel": "CH234", "text": expected_text}
...
def test_slack_inst_should_upload_latest_plot(mock_webclient, slack, mocker):
    method_name = "qcodes_loop.extensions.slack.BasePlot.latest_plot"
...
def test_slack_inst_should_print_measurement(mock_webclient, slack, mocker):
    ...
    mocker.patch("qcodes_loop.extensions.slack.active_data_set", return_value=dataset)
