...
def test_loop_simple(dci):
    ...
    assert_array_equal(
        data.dci_ChanA_temperature_set.ndarray, data.dci_ChanA_temperature.ndarray
    )
...
def test_loop_measure_channels_by_name(dci, values):
    ...
    for i, chan in enumerate(["A", "B", "C", "D"]):
        assert getattr(data, f"dci_Chan{chan}_temperature").ndarray.shape == (21,)
        assert getattr(data, f"dci_Chan{chan}_temperature").ndarray.max() == values[i]
        assert getattr(data, f"dci_Chan{chan}_temperature").ndarray.min() == values[i]


@given(
    loop_channels=hst.lists(hst.integers(0, 3), min_size=2, max_size=2, unique=True),
...
def test_nested_loop_over_channels(dci, loop_channels, measure_channel):
    ...
    assert getattr(
        data, f"dci_Chan{channel_to_label[loop_channels[0]]}_temperature_set"
    ).ndarray.shape == (21,)
    assert getattr(
        data, f"dci_Chan{channel_to_label[loop_channels[1]]}_temperature_set"
    ).ndarray.shape == (
        21,
        11,
    )
    assert getattr(
        data, f"dci_Chan{channel_to_label[measure_channel]}_temperature"
    ).ndarray.shape == (21, 11)
